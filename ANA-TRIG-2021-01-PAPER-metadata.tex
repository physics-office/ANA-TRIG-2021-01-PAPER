...
Electron trigger identification is a crucial input to many ATLAS physics analysis. To cope with the increase of peak LHC luminosity from 2017 to 2018 that pushed the number of proton-proton interactions per beam-crossing to about 60, trigger algorithms were optimized to keep the background trigger rates low while retaining a high efficiency for physics analyses. 
In the second half of 2017, the Neural-Ringer algorithm, which is a set of Multi-Layer Perception (MLP) models, replaced the cut-based strategy in the FastCalo decision step and became the baseline algorithm for triggering events containing electrons above 15 GeV in the High Level Trigger (HLT), as part of the ATLAS online system CPU reduction campaign. 
